  ...
  return (
    <div className="absolute bg-black w-full h-full left-0 top-0 bg-opacity-30 flex justify-center">
      <div className="place-self-center w-3/4 h-3/4 bg-gray-50 rounded-lg flex flex-col px-5 py-3 gap-3 relative">
        {showQR ? (
          <div className="absolute z-10 right-0 top-0 w-full h-full p-5 rounded-md border bg-white bg-opacity-60 backdrop-blur-md flex flex-col justify-between">
            <div className="place-self-end">
              <button onClick={() => setShowQR(false)}>
                <IoClose size={40} color="gray" />
              </button>
            </div>
        ...
        <div className="flex justify-between">
          ...
          <button
            className="px-2 text bg-yellow-200 font-medium rounded"
            onClick={() => {
              setShowQR(true);
            }}
          >
